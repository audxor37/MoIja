...
create table matches (
  ...
  recurrence_count integer not null default 1 check (recurrence_count between 1 and 12),
  created_by uuid not null references profiles(id),
  created_at timestamptz not null default now()
);
...
create index if not exists matches_recurrence_group_id_idx on matches (recurrence_group_id) where recurrence_group_id is not null;
...
begin
  ...
  if input_repeat_count < 1 or input_repeat_count > 12 then
...
revoke all on function public.create_match_series_with_default_attendances(text, timestamptz, integer, attendance_method, integer, text, text, boolean, integer, text[]) from public;
grant execute on function public.create_match_series_with_default_attendances(text, timestamptz, integer, attendance_method, integer, text, text, boolean, integer, text[]) to authenticated;
